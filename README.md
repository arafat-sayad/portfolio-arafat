...
const greeting = {
  ...
  subTitle: emoji("Cloud System Administrator & DevOps Engineer"),
...
- Navigate to `package.json` and enter your domain name instead of `http://34.73.195.205/` in `homepage` variable. For example, if you want your site to be `https://<your-username>.github.io/Arafat-folio`, add the same to the homepage section of `package.json`.

- In short you can also add `/Arafat-folio` to `package.json` as both are exactly same. Upon doing so, you tell `create-react-app` to add the path assets accordingly.
...
## For the Future
If you can help us with these. Please don't hesitate to open a [pull request](https://github.com/arafat-sayad/protfolio.git).
